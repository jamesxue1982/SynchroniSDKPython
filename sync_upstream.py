...
测试代码提交在 Testing_James 分支上。当上游发布新版本时，本脚本只负责把
upstream 的默认分支（master）同步到本地 master，不直接操作 Testing_James。
之后在 Testing_James 分支上手动执行 `git merge master`（或 `git merge origin/master`）
即可把最新代码/example/README 合入测试分支，同时保留测试代码。
