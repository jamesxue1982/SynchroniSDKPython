...
对应用例：03_数据流.md -> DATA-BND-001
可自动化：auto（设备上电、在范围内为运行前置，测试中无需人工动作）

说明：
  验证 init 的 packageSampleCount 参数边界。init 是 C 扩展实现，非法参数可能
...
边界值（MAX 假设 int32 上限，README 未文档化 packageSampleCount 上限，实测若有
文档化上限请调整）：
  1、2、MAX-1、MAX、0、MAX+1

判定（与 CTRL-BND-001 一致）：
  - 合法值（1/2/MAX-1/MAX）：不崩溃、不抛异常即通过（返回 True/False 均记录；
    MAX-1/MAX 若返回 False 可能是超出 SDK 实际上限，标注"需确认"）
  - 非法值（0/MAX+1）：被拒绝（返回 False 或抛异常）即通过；返回 True 判 FAIL
...
MAX_INT32 = 2147483647  # packageSampleCount 假设上限（int32 max）
BOUNDARY_VALUES = [1, 2, MAX_INT32 - 1, MAX_INT32, 0, MAX_INT32 + 1]
LEGAL = {1, 2, MAX_INT32 - 1, MAX_INT32}
...
# 子进程探针：完整走 scan->match->connect->Ready->init(value) 并打印结果
PROBE = r'''
import sys, time
from sensor import *
...
def _parse(out):
    ...
    kind: 'return'/'raised'/'notarget'/'noprofile'/'notready'/'scanraised'/'connectraised'/'unknown'。"""
...
def main():
    print("=" * 60, flush=True)
    print("DATA-BND-001 init(packageSampleCount) 6 点边界", flush=True)
    print("=" * 60, flush=True)
    ...
    for i, value in enumerate(BOUNDARY_VALUES):
        ...
        print(f"\n[{i+1}/{len(BOUNDARY_VALUES)}] 测试 packageSampleCount = {value}（{tag}）...", flush=True)
