...
async def main_async():
    ...
    if isinstance(async_battery, int) and isinstance(sync_battery, int):
        match = (async_battery == sync_battery)
        print(f"[比较] asyncGetBatteryLevel() == getBatteryLevel() ? {match}", flush=True)
        record(results, "asyncGetBatteryLevel 与同步 getBatteryLevel 结果一致", match,
               "asyncGetBatteryLevel 与 getBatteryLevel 返回相同值",
               f"async={async_battery} sync={sync_battery}")
    else:
