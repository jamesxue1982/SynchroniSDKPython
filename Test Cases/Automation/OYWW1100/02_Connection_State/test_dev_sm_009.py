...
流程：
  1) 确认设备开机 -> 按回车
  2) scan -> requireSensor -> 显式设置 autoReconnect=False 并断言读回
  3) connect 到 Ready -> init -> startDataNotification 起流
  4) 人工断电/移出范围 -> 观察断链（Disconnected + onErrorCallback）
  5) 人工恢复供电/移回范围 -> 观察窗口内确认【不】自动重连（未回到 Ready、onAutoReconnect 0 次）
"""
...
def main():
    ...
    print("=" * 60, flush=True)
    print("DEV-SM-009 autoReconnect=False 不自动重连", flush=True)
    print("=" * 60, flush=True)
    ...
    try:
        ...
        set_txt = f"读回 {auto_reconnect}"
    except Exception as e:
        auto_reconnect = None
        set_txt = f"抛异常 {type(e).__name__}: {e}"
    print(f"[检查] SensorProfile.autoReconnect=False 后读回 = {auto_reconnect}", flush=True)
    record(results, "autoReconnect 可写为 False 且生效", auto_reconnect is False,
           "autoReconnect == False", f"autoReconnect -> {set_txt}")
    ...
    # ---- 阶段 1：人工断电/移出范围，观察断链 ----
    input("\n>>> [人工操作] 请【关闭】待测设备 OYWW1100 电源（或移出蓝牙范围），完成后按回车继续 ...")

    print(f"\n[等待断链] 最多 {DISCONNECT_TIMEOUT}s ...", flush=True)
    # 检查点：状态变化到 Disconnected（观察断链事件）
    ...
    # 检查点：onErrorCallback 触发（异常断链原因上报）
    ...
    # ---- 阶段 2：人工恢复供电，观察是否【不】自动重连 ----
    input("\n>>> [人工操作] 请【重新开启】待测设备 OYWW1100 电源（或移回范围），完成后按回车继续 ...")

    print(f"\n[观察不自动重连] 观察窗口 {NO_RECONNECT_WINDOW}s ...", flush=True)
    reached_ready = False
    t0 = time.time()
    while time.time() - t0 < NO_RECONNECT_WINDOW:
        if sensor.deviceState == DeviceStateEx.Ready:
            reached_ready = True
            print("  [观察] 检测到 deviceState==Ready（出现自动重连），提前结束观察", flush=True)
            break
        time.sleep(0.5)

    print(f"[观察] deviceState={sensor.deviceState} onAutoReconnect {len(reconnects)} 次 状态序列 {[str(s) for s in states]}", flush=True)

    record(results, "恢复供电后未自动重连（deviceState 保持 Disconnected）", not reached_ready,
           "观察窗口内 deviceState 不回到 Ready",
           f"state={sensor.deviceState} 状态序列={[str(s) for s in states]}")
    record(results, "onAutoReconnect 未触发（0 次）", len(reconnects) == 0,
           "onAutoReconnect 触发 0 次",
