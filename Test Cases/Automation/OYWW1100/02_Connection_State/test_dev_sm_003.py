...
可自动化：auto（需环境中存在 ≥2 台设备，其一为目标 OYWW1100，其余为"非目标"）

前置条件：
  - 主机(电脑)：蓝牙已开启
  - 待测设备（目标）：OYWW1100 上电在范围内
  - 非目标设备：OB6000C 上电在范围内（用于验证"不误连"）

流程：
  1) 确认设备开机 -> 按回车
  2) scan 列出所有设备，识别目标 OYWW1100 与其它（非目标）设备
...
def main():
    ...
    if target is None:
        print("[FAIL] 未匹配到 config 中启用的目标设备（OYWW1100/80F3）", flush=True)
    ...
    record(results, "scan 匹配到目标设备", True, "scan 返回含 OYWW1100", f"匹配到 {target_name} {target_addr}")
    ...
    if not connected:
        ...
        record(results, "连接集合仅含目标设备（不误连）", None,
               "getConnectedSensors/Devices 仅含 OYWW1100", "未连接成功，跳过防串验证")
