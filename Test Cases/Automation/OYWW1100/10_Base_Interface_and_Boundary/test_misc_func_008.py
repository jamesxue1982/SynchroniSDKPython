...
流程：
  1) 确认设备开机 -> 按回车
  2) scan 匹配 OYWW1100 -> requireSensor -> connect -> init
  3) getDeviceInfo() 获取 DeviceInfo 对象
  4) 逐一读取 ModelName 及各模态 ChannelCount/SampleRate
  5) 断言字段存在且值 ≥0；OYWW1100 的 EMG/IMU 类字段 >0，EEG/ECG 类为 0
"""
...
from sensor import *
import config
from common import record, match_target
...
# OYWW1100 预期：EMG/IMU/Acc/Gyro 类字段 >0
    ...
    for f in OYWW_EXPECTED_POSITIVE:
        ...
        if isinstance(v, (int, float)):
            if v <= 0:
                ...
                positive_ok = False
    record(results, "OYWW1100 预期字段（EMG/IMU/ACC/GYRO 等）>0", positive_ok,
           "OYWW1100 的 EMG/IMU/Acc/Gyro 等字段 >0",
           "全部 >0" if positive_ok else "存在 ≤0")

    # 检查4：OYWW1100 预期 =0 的字段（EEG/ECG/PPG/SPO2/BRTH）
    zero_ok = True
    for f in OYWW_EXPECTED_ZERO:
        ...
        if isinstance(v, (int, float)):
            if v != 0 and v is not None:
                ...
                zero_ok = False
    record(results, "OYWW1100 非预期字段（EEG/ECG/PPG/SPO2/BRTH）=0", zero_ok,
           "OYWW1100 的 EEG/ECG/PPG/SPO2/BRTH 等字段 =0",
