...
对应用例：04_参数.md -> PARAM-FUNC-008
可自动化：auto（设备上电、在范围内为运行前置）

流程：
  1) scan -> requireSensor -> connect -> 到达 Ready -> init
  2) setParam("UNKNOWN_KEY", "ON")，校验返回以 "Error" 开头

说明：
  README：key 不支持时结果以 "Error" 开头。
...
def main():
    ...
    print("=" * 60, flush=True)
    print("PARAM-FUNC-008 未知 key 返回 Error 开头", flush=True)
    print("=" * 60, flush=True)
    ...
    # 未知 key
    print(f"\n[参数] setParam({UNKNOWN_KEY!r}, 'ON') ...", flush=True)
    try:
        r = sensor.setParam(UNKNOWN_KEY, "ON")
    except Exception as e:
        r = f"抛异常 {type(e).__name__}: {e}"
    print(f"[参数] setParam -> {r!r}", flush=True)

    is_error = isinstance(r, str) and r.startswith("Error")
    record(results, f"setParam({UNKNOWN_KEY!r}) 返回 Error 开头",
           is_error,
           "返回以 'Error' 开头",
           f"setParam->{r!r}")
