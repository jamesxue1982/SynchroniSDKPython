...
前置条件：
  - 主机(电脑)：蓝牙已开启
  - 待测设备：OB6000C 需先开机（用于扫描获取 BLEDevice），再关闭电源制造连接失败

流程：
  1) 确认设备开机 -> 按回车
  2) scan 匹配 OB6000C -> 记录 BLEDevice
...
def main():
    ...
    print("  - 待测设备：OB6000C 需先开机（扫描获取 BLEDevice），再关闭电源制造连接失败", flush=True)
    ...
    # ---- 阶段 2：人工关闭设备，制造连接失败 ----
    input("\n>>> [人工操作] 请【关闭】待测设备 OB6000C 电源（或将其移出蓝牙范围），完成后按回车继续 ...")
